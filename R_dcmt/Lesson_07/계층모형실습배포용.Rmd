...
library(Hmisc) #데이터프레임에 hist를 적용하기 위해 필요. 
...
m = 100000
theta = rbeta(m, 8, 4)
data.frame(theta=theta) %>% ggplot(aes(x=theta)) + geom_histogram()
data.frame(theta=theta) %>% ggplot(aes(x=theta)) + geom_density()
```
...
data.frame(theta=theta) %>% mcmc %>% summary()
```
...
pin.fit = stan(model_code=pin.code, data=data, 
               seed=1234567, chains=2, iter=50000, thin=10)
```
...
```{r}
x = c(186.5, 184.7, 170.4, 179.8, 169.1, 176.6, 175.1, 176.6, 186.8, 170.2)
n = 10
data = list(x=x, n=n)
```
...
model {
  for(i in 1:n) {
    target += normal_lpdf(x[i] | theta, sigma);
  }
}
"
```
...
```{r}
height.fit = stan(model_code=height.code, data=data, 
               seed=1234567, chains=2, iter=50000, thin=10)
```
...
plot(height.fit, plotfun="plot")
plot(height.fit, plotfun="dens")
plot(height.fit, plotfun="hist")
plot(height.fit, plotfun="trace")
plot(height.fit, plotfun="ac", pars="theta")
```
